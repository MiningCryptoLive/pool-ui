// config
var API = 'https://etcsolo.miningcrypto.live/api/'; // API address
...
function loadStatsData() {
	...
	$.each(data.pools, function(index, value) {
		...
		$("#totalPaid").text(_formatter(value.totalPaid, 2,"") + "ETC");
    ...
    return $.ajax(API + 'pools/' + currentPool + '/miners')
        .done(function (data) {
            ...
            if (data.length > 0) {
                $.each(data, function (index, value) {
                    minerList += '<tr>';
                    minerList += '<td>' + value.miner.substring(0) + ' &hellip; ' + value.miner.substring(value.miner.length - 0) + '</td>';
...
function loadConnectConfig() {
    return $.ajax(API + 'pools')
        .done(function (data) {
            ...
            $.each(data.pools, function (index, value) {
                if (currentPool === value.id) {
                    connectPoolConfig += '<tr><td>Algorithm</td><td>EtcHash</td></tr>';
